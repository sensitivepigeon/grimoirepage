...
const QUEST_SCHEMA = [
  ...
  { key: "requires_quest", label: "Requires quest", type: "text",     required: false,
    help: "Quest ID that must be completed first, e.g. tier1/some_quest. Blank for none." },
];
  ...
  QUEST_SCHEMA.forEach((field) => {
    ...
    switch (field.type) {
      ...
      case "textarea":
        ...
        break;
      case "int":
        quest[field.key] = document.getElementById(id).valueAsNumber; // NaN if blank
        break;
      case "select":
...
function validate(quest) {
  ...
  const reward = quest.rewards[0];      // Phase 1 always has exactly one
...
function buildJson(quest) {
  ...
  out.title = quest.title;                                  // required

  if (quest.lore) out.lore = quest.lore;                    // optional strings:
  if (quest.description) out.description = quest.description;//   only if typed
  if (quest.patron) out.patron = quest.patron;

  if (quest.format && quest.format !== 1) out.format = quest.format;  // only if changed

  out.tier = quest.tier;                                    // required
  out.required_item = quest.required_item;
  out.required_count = quest.required_count;

  out.rewards = quest.rewards;                              // always an array

  if (quest.repeatable === false) out.repeatable = false;   // only when false
  if (quest.requires_quest) out.requires_quest = quest.requires_quest; // only if typed
